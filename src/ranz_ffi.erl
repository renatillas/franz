...
-export([start_client/2, start_producer/1, produce_sync_offset/4, produce_sync/4]).
...
produce_sync(TopicName, Partition, Key, Value) ->
  case brod:produce_sync_offset(brod_default_client, TopicName, Partition, Key, Value) of
    ok ->
      {ok, nil};
    {error, Error} ->
    {error, Error} -> 
